...
function oracionTo15ai(oracion, oracionEnIngles = "") {
  ...
  let previoEraPuntuacion = true,
    ...
    puntuaciones = /\p{P}/u,
    añadirPalabra = (palabra, puntuacion = "") =>
      (resultado += "{" + PalabraUtils.toARPAbet(palabra) + "}" + puntuacion);
  ...
  for (let pedazo of pedazos) {
    ...
    if (palabraActual) añadirPalabra(palabraActual);
  }
  oracionEnIngles = oracionEnIngles.trim();
  return oracionEnIngles ? resultado + "|" + oracionEnIngles : resultado;
}
...
const PalabraUtils = {
    ...
    for (let actPos = 0; actPos < palabra.length; ) {
      ...
      if (actPos < palabra.length - 1)
        if (palabra[actPos] == "u") {
          if (
            ultimaConsonante == "q" ||
            (ultimaConsonante == "g" && "eéií".includes(palabra[actPos + 1]))
          )
            actPos++;
        } else if (palabra[actPos] == "ü" && ultimaConsonante == "g")
          ...
          if (actPos < palabra.length) {
            ...
            if ("áàéèóò".includes(char)) {
              ...
              if (anterior) {
                ...
                esTonica = true;
                actPos++;
              } else if (hache) actPos--;

              seguir = false;
            } else if ("iu".includes(char)) {
              // Vocal débil
              if (
                actPos < palabra.length - 1 &&
                !this.esConsonante(palabra[actPos + 1])
              ) {
                // ¿Hay tercera vocal?
                if (palabra[actPos - 1] == "h") actPos--;
      ...
      actPos = ((pos) => {
        ...
        if (!this.esConsonante(palabra[pos + 1])) return pos;
        ...
        if (pos < palabra.length - 2) {
          ...
          if (!this.esConsonante(c3)) {
            ...
            if (c2 == "y") return "slrnc".includes(c1) ? pos : pos + 1;

            // gkbvpft + l
            if ("gkbvpft".includes(c1) && c2 == "l") return pos;

            // gkdtbvpf + r
            if ("gkdtbvpf".includes(c1) && c2 == "r") return pos;

            return pos + 1;
          } else {
            // Hay tercera consonante
            if (pos + 3 == palabra.length) {
            ...
            if (c2 == "y")
              ...
              return !"slrnc".includes(c1) ? pos + 1 : pos;
            ...
            return (
              pos +
              (c3 == "l" ||
              c3 == "r" || // Los grupos consonánticos formados por una consonante seguida de 'l' o 'r' no pueden separarse y siempre inician sílaba
              (c2 == "c" && c3 == "h") || // 'ch'
        ...
        } else if (c2 != "y") return pos + 2; // La palabra acaba con dos consonantes
        return pos;
      })(actPos);
    ...
    if (!encontroTonica) {
      ...
      // Para diferenciar monosílabos con tilde diacrítica.
        ...
        if (
          ...
          letraFinal == "y" ||
          letraFinal == "n" ||
          (letraFinal == "s" && !this.esConsonante(palabra[palabra.length - 2]))
        )
  ...
  toARPAbet: function (palabra) {
    let silabas = this.getSilabas(palabra),
      ...
      defaultAdd = (letra) =>
        ARPAbet.push(reemplazos[letra] || letra.toUpperCase());
    for (let [i, silaba] of Object.entries(silabas)) {
      //reemplazar letras por sonidos
      silaba.texto = silaba.texto.replace(
        /(ch)|(ll)|(h)|(c([aou]))|(c([ei]))|(qu)|(g([ei]))|(v)|(rr)|(gu([ei]))/g,
        (
          str,
          ch,
          ll,
          ...
          if (ll) return "y";
          if (h) return "";
          if (cacocu) return "k" + aou;
          if (ceci) return "s" + eic;
          if (qu) return "k";
          if (gegi) return "j" + eig;
          if (v) return "b";
          if (rr) return "r";
          if (guegui) return "g" + eigu;
        }
      );
      ...
      for (let [j, letra] of Object.entries(silaba.texto)) {
        if ("aeiou".includes(letra)) {
          ...
          if (!(i + 1 == silabas.length && j + 1 == silaba.length)) {
            let next = j + 1 == silaba.length ? silabas[i][0] : silaba[j + i];
            if (next == "k" || next == "g") {
              ARPAbet.push("NG");
              continue;
            }
...
/**
 * Convierte fonemas de CMUdict (con números de estrés) a ARPABET puro
 * @param {string} cmudictPhonemes - Fonemas en formato CMUdict (ej: "HH AH0 L OW1")
 * @returns {string[]} - Array de fonemas ARPABET sin números de estrés
 */
function cmudictToArpabet(cmudictPhonemes) {
  if (!cmudictPhonemes || typeof cmudictPhonemes !== "string") {
    return [];
  }

  // Dividir por espacios y quitar números de estrés (0, 1, 2) y llaves

  return cmudictPhonemes
    .trim()
    .replace(/[{}]/g, "") // Remove curly brackets
    .split(/\s+/)
    .map((phoneme) => phoneme.replace(/[012]$/, ""));
}

function convertSpanishWordToPhonemes(word) {
  const cmudictPhonemes = PalabraUtils.toARPAbet(word);
  return cmudictToArpabet(cmudictPhonemes);
}
